...
FRAMES_PER_SNAPSHOT = 2

RUN_TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
BASE_DIR = f"LiDAR_Dataset_Setting2_{RUN_TIMESTAMP}"

ATTACK_MIN_FRAMES = 4
ATTACK_MAX_FRAMES = 16
...
def main():
    ...
    state = {
        ...
        'current_sequence': 1,
        'finished': False,
        'attack_start': random.randint(30, 100),
        'attack_end': 0,
        'ghost_actor': None,
        'ghost_xyz': None
    }
    ...
    def callback(data):
        ...
        if len(state['frame_buffer']) == FRAMES_PER_SNAPSHOT:
            ...
            if state['current_snapshot'] > SNAPSHOTS_PER_SEQUENCE:
                ...
                state['attack_start'] = random.randint(30, 100)
                state['attack_end'] = state['attack_start'] + random.randint(ATTACK_MIN_FRAMES, ATTACK_MAX_FRAMES)
