...
def save_worker(save_queue, stop_event):
    """
    Background thread that saves the full point clouds, the isolated ghost points,
    and updates the labels.csv ground truth file simultaneously.
    """
    while not stop_event.is_set() or not save_queue.empty():
        try:
            ...
            with open(csv_path, 'a', newline='') as csvfile:
                ...
                writer.writerow([filename, status, gx, gy, gz, ghost_filename])
...
def main():
    ...
    def callback(data):
        ...
        if len(state['frame_buffer']) == FRAMES_PER_SNAPSHOT:
            ...
            if is_spoofed:
                ...
                # Create a boolean mask to filter the numpy array
            ...
            save_queue.put((
                ...
                is_spoofed, 
                state['ghost_xyz'],
                ghost_points # Pass the isolated points to the save worker
            ))
    ...
    print(f"\nRECORDER ACTIVE: Saving to {BASE_DIR}")
    print("-------------------------------------------------------------------------")
